...
export default class PropertyEditor extends React.PureComponent {
	...
	render() {
		const {
			...
			disabled,
			rowClassName, labelClassName, keyClassName, valueClassName,
		...
		const rows = properties.map(property => {
			...
			const { key, caption, editor: Component, className: propClassName } = property;
			const hasComplaint = complaints && key in complaints;
			const classNames = classnames(
				rowClassName, propClassName, { [hasComplaintClassName]: hasComplaint }
			);
			const row = [
