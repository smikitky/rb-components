...
const formControl = (options, baseFilter) => {
	...
	const result = ({ onChange, value = '', disabled }) => <FormControl
		{...otherOptions}
		value={value}
...
export const checkbox = options => {
	return ({ onChange, value, disabled }) => <span>
		<Checkbox
			checked={!!value}
...
export const shrinkSelect = options => {
	return props => <ShrinkSelect
		options={options}
		value={props.value}
		onChange={props.onChange}
		disabled={props.disabled}
...
export const arrayOf = (editor, newItemValue = '', inline) => {
	const result = props => <ArrayEditor
		editor={editor}
