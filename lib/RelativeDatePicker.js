...
function dateToString(input) {
	...
	return (delta < 0) ? `${-delta} ${unitStr} ago` : `${delta} ${unitStr} from now`;
}
...
export default class RelativeDatePicker extends React.PureComponent {
	...
	setMode(mode) {
		...
		if (mode === 'relative') {
			...
			this.triggerChange(moment().format(format));
		}
		this.setState({ mode });
	}
	...
	render() {
		...
		const { value, disabled, bsSize = undefined, bsStyle = 'default', format } = this.props;
				...
				<div className='switch'>
					...
					:
						<Calendar
							format={format}
							value={value}
							onChange={date => this.dateClick(date)}
						/>
					}
				</div>
			}
