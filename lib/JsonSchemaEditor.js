...
class ValidatingTextProp extends React.PureComponent {
  ...
  handleChange(ev) {
    ...
    if (this.isValid(val.length ? val : undefined, validator)) {
      ...
      onChange(keyName, val.length ? val : undefined);
    } else {
  ...
  handleTextBlur(ev) {
    const { onTextBlur, keyName } = this.props;
...
const EnumProp = props => {
  ...
  const handleChange = ev => {
    ...
    onChange(keyName, index >= 1 ? options[index - 1] : undefined);
  };
...
const BooleanProp = props => {
  ...
  const handleChange = ev => onChange(keyName, ev.target.checked);
  return (
    <input
      type="checkbox"
      checked={!!value}
